...
public class XMLReader {
	...
	private static final String GEOCODERESPONSE_NODE = "GeocodeResponse";
	...
	public Place[] readSearchResult(InputStream inputStream) throws XmlPullParserException, IOException {
		...
		parser.nextTag();
		List<SimplePlace> placesVector = new ArrayList<>();
		while (RESULT_NODE.equals(parser.getName())) {
			parser.require(XmlPullParser.START_TAG, null, RESULT_NODE);
			parser.nextTag();
			List<String> typesVector = new ArrayList<>();
			while (TYPE_NODE.equals(parser.getName())) {
				typesVector.add(XmlPullParserHelper.parseSimpleNodeWithText(parser, TYPE_NODE));
				parser.nextTag();
			}
			String[] types = typesVector.toArray(new String[typesVector.size()]);
			String formattedAddress = XmlPullParserHelper.parseSimpleNodeWithText(parser, FORMATTED_ADDRESS_NODE);
			parser.nextTag();
			List<SimpleAddressComponent> addressesComponentsVector = new ArrayList<>();
			while (ADDRESS_COMPONENT_NODE.equals(parser.getName())) {
				...
				parser.nextTag();
				List<String> subtypesVector = new ArrayList<>();
				while (TYPE_NODE.equals(parser.getName())) {
					subtypesVector.add(XmlPullParserHelper.parseSimpleNodeWithText(parser, TYPE_NODE));
					parser.nextTag();
				}
				parser.require(XmlPullParser.END_TAG, null, ADDRESS_COMPONENT_NODE);
				String[] subtypes = subtypesVector.toArray(new String[subtypesVector.size()]);
				SimpleAddressComponent addressComponent = new SimpleAddressComponent(shortName, longName, types);
				addressesComponentsVector.add(addressComponent);
				parser.nextTag();
			}
			...
			XmlPullParserHelper.parseSimpleNodeWithText(parser, "place_id");
			parser.nextTag();
			parser.require(XmlPullParser.END_TAG, null, RESULT_NODE);
			AddressComponent[] addressComponents = addressesComponentsVector
					.toArray(new SimpleAddressComponent[addressesComponentsVector.size()]);

			SimplePlace place = new SimplePlace(coordinates, formattedAddress, addressComponents);
		...
		Place[] result = placesVector.toArray(new SimplePlace[placesVector.size()]);
